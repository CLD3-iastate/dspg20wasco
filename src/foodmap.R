...
addLegendCustom <- function(map, colors, labels, sizes, opacity = 0.5, group = "Stores"){
 ...
 leaflet() %>%
    ...
    addCircleMarkers(data = food_points,
                     ...
                     group = "Stores") %>%
    addLayersControl(
      baseGroups = c("Basemap", "Stores"),
      overlayGroups = c("Driving times"),
    ...
    addLegend(data = unincorporated, "topright", opacity = 0.4,
              ...
              group = "Basemap") %>%
    addLegend(data = isochrones, pal = isochronepal,
              values = ~value, labels = c("30 minute drive", "1 hour drive"),
              group = c("Driving times"))
...
saveWidget(foodmap, file = "testing.html")
