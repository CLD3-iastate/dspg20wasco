...
wic <- esri2sf(url)
wic <- st_as_sf(wic)
## Oregon live grocery stores
oregon_live <- read_csv("~/git/DSPG2020/wasco/data/oregon_live.csv")
oregon_live <- oregon_live %>% geocode(address, method = "census")
oregon_live <- oregon_live %>% drop_na()
oregon_live_sf <- oregon_live %>% st_as_sf(coords = c(x = "long", y = "lat"), crs = 4326, agr  = "constant")
plot(oregon_live)

## USDA local food
local_food <- read_csv("~/git/DSPG2020/wasco/data/usda_local_food_oregon.csv")
local_food <- local_food %>% select("MarketName", "x", "y") %>% drop_na() %>% st_as_sf(coords = c(x = "x", y = "y"), crs = 4326, agr  = "constant")
plot(local_food)
...
ggplot() +
  ...
  geom_sf(data = big_streets$osm_lines,
          ...
          size = .5,
          alpha = .6) +
  geom_sf(data = snap, color = "blue") +
  geom_sf(data = wic, color = "green") +
  geom_sf(data = oregon_live_sf, color = "yellow", position = "identity") +
  geom_sf(data = local_food, color = "purple", position = "identity") +
  xlim(122, 120) +
  ylim(44.5, 45.75)
...
leaflet() %>%
  ...
  addCircles(data = buildings$osm_points)
