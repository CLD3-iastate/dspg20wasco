...
p <- ggplot(acs_counties %>% mutate(south_wasco = fct_other(NAME, keep = c("South Wasco County School District 1, Oregon", "Wasco County, Oregon", "Oregon"), other_level = "Neighboring Counties"))
  ...
  geom_line(size = 1) + 
  geom_point(size = 2.5) +
  scale_color_manual(name = "Region", values = c(viridis(3, option = "D"), graypal), labels=c("Oregon", "South Wasco", "Wasco", "Neighboring Counties")) +
...
```{r}
ggplot(filter(race_table,year ==2018), aes(value, NAME, color = variable)) +
  geom_point(size = 5) +
  ggtitle("Percent of race/ethnicity per region") + ylab("") + 
    xlab("Percent of Population") +theme_minimal() +
  scale_color_discrete(name = "Racial or Ethnic Group", 
