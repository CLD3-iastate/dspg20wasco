...
```{r}
top_10_over_time <- inner_join(trimmed_2017, agg_cnty_OR_with_names_2016, by = "ctyname")
top_10_over_time <- top_10_over_time %>%
  select(h_county.x, ctyname, S000.x, S000.y)
top_10_over_time$'2017' <- top_10_over_time$S000.x
top_10_over_time$'2016' <- top_10_over_time$S000.y
top_10_over_time <- inner_join(top_10_over_time, agg_cnty_OR_with_names_2015, by = "ctyname") %>%
  select(h_county.x, ctyname, '2017', '2016', S000)
top_10_over_time$'2015' <- top_10_over_time$S000
top_10_over_time$h_county <- top_10_over_time$h_county.x
top_10_over_time <- top_10_over_time %>% select(-S000, -h_county.x)
top_10_over_time <- top_10_over_time %>% select(ctyname, '2017', '2016', '2015')
top_10_trans <- data.frame(t(top_10_over_time[, -1]))
colnames(top_10_trans) <- top_10_over_time$ctyname
colnames(top_10_trans) <- c("HoodRiver", "Multnomah", "Clackamas", "Marion", "Washington", "Deschutes", "Jefferson", "Lane", "Umatilla", "Sherman")
top_10_trans$year <- c(2017, 2016, 2015)
#line plot for all jobs 2015-2017 by county
ggplot(top_10_trans, aes(x = year)) +
  ggtitle("Number of jobs flowing into Wasco County\nfrom other counties in Oregon from\n2015-2017") +
  labs(x = "Year", y = "Number of Jobs", colour = "County") + 
  geom_line(aes(y = HoodRiver, color = "HoodRiver")) + 
  geom_line(aes(y = Multnomah, color = "Multnomah")) + 
  geom_line(aes(y = Clackamas, color = "Clackamas")) + 
  geom_line(aes(y = Marion, color = "Marion")) + 
  geom_line(aes(y = Washington, color = "Washington")) + 
  geom_line(aes(y = Deschutes, color = "Deschutes")) + 
  geom_line(aes(y = Jefferson, color = "Jefferson")) + 
  geom_line(aes(y = Lane, color = "Lane")) + 
  geom_line(aes(y = Umatilla, color = "Umatilla")) + 
  geom_line(aes(y = Sherman, color = "Sherman"))
...
outflows over time!!
```{r}
top_10_over_time <- inner_join(trimmed_out_2017, agg_out_names_2016, by = "ctyname")
top_10_over_time <- top_10_over_time %>%
...
top_10_over_time$'2015' <- top_10_over_time$S000
top_10_over_time$h_county <- top_10_over_time$w_county.x
top_10_over_time <- top_10_over_time %>% select(-S000, -w_county.x)
top_10_over_time <- top_10_over_time %>% select(ctyname, '2017', '2016', '2015')
top_10_trans <- data.frame(t(top_10_over_time[, -1]))
colnames(top_10_trans) <- top_10_over_time$ctyname
colnames(top_10_trans) <- c("HoodRiver", "Multnomah", "Clackamas", "Deschutes", "Washington", "Marion", "Jefferson", "Umatilla", "Skamania", "Lane")
top_10_trans$year <- c(2017, 2016, 2015)
#line plot for all jobs 2015-2017 by county
ggplot(top_10_trans, aes(x = year)) +
  ...
  geom_line(aes(y = Lane, color = "Lane")) + 
  geom_line(aes(y = Umatilla, color = "Umatilla")) + 
  geom_line(aes(y = Skamania, color = "Skamania"))
...
#** get polygons (Virginia block group spatial(sf) files) ---- >> HAVE FOR WASCO
...
#** get list of county fips codes ---- >>>> ONLY ONE FOR WASCO
...
raster_2017
#st_as_sf(read_stars("../data/lodes/raster/CDL_2017_41065.tif"), as_points = TRUE, merge = FALSE)
```

takes forever to run (using 8 cores)
```{r}
cdl_sf_2017 <- st_as_stars(raster_2017)%>% 
  st_as_sf(merge = TRUE)
```
